...
def scrap_idle(intent: dict, db: Session) -> dict:
    ...
    deadline = china_now() - timedelta(days=days)
...
def drawing_recent(intent: dict, db: Session) -> dict:
    days = int((intent.get("time_range") or {}).get("days") or intent.get("extra") or 30)
    drawings = db.query(ProductDrawing).filter(ProductDrawing.created_at >= china_now() - timedelta(days=days)).order_by(ProductDrawing.created_at.desc()).limit(50).all()
