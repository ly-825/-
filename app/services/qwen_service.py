...
11. tooth_type = 齿型前缀，只能是 IT、IL、IR、OT、OL、OR 之一；例如 OT48(52) 中 tooth_type 为 OT。
12. teeth_count = 齿数 z 的数值部分；如果是 48(52)，teeth_count 填 48。
13. teeth_count_text = 齿数原始显示文本，允许 48(52) 这种带括号的特殊表示。
14. module = 公制模数数值；如果是 DP、CP 等英制/字母模数，module 填 null。
15. module_text = 模数原始显示文本，允许 DP、CP、P 等英文字母。
16. pressure_angle = 压力角 α，常见值 20。
17. profile_shift_coefficient = 变位系数 x。
18. span_teeth_count = 跨齿数 k/n。
19. common_normal_length = 公法线长度 W/L 的数值；如果图纸是范围，外齿 O* 取大值，内齿 I* 取小值。
20. common_normal_length_text = 公法线长度原始显示文本，允许 58.26-58.14 这种范围。
21. pin_diameter = 标准量棒/量棒直径 dp。
22. pin_span = M值/棒间距 M（量棒测量值）。
23. expected_scrap_theoretical_size = 圈中间割下来的那一块中心圆料尺寸，按内孔/中心孔直径计算，不是产品外径。
24. expected_scrap_usable_size 应保守计算，通常比中心圆料理想直径小 0.5mm。
25. confidence 取 0 到 1。
26. 如果关键信息缺失，need_manual_review 为 true。

常见关键词对应：
- 齿型+齿数，例如 IT41、OL4、OT48(52) → tooth_type + teeth_count_text + teeth_count
- 齿数/z/Z → teeth_count_text，能转数字时同时填 teeth_count
- 模数/m/M/模 → module_text，纯数字时同时填 module
- 压力角/α → pressure_angle
- 变位系数/x → profile_shift_coefficient
- 跨齿数/k/n → span_teeth_count
- 公法线/W/Wk/L → common_normal_length_text，纯数字或范围按齿型取值后填 common_normal_length
...
- 裸整数 10~300 且出现在普通文本中，优先作为 teeth_count 候选；如果只有一个这样的整数，可以填 teeth_count 和 teeth_count_text。
- 48(52) 表示实际齿数 48、理论/原始齿数 52，teeth_count_text 必须保留 48(52)。
- IT/IL/IR/OT/OL/OR 是齿型前缀，不要丢弃。
...
输出字段：product_code, product_name, material, product_thickness, plate_thickness, max_outer_diameter, inner_related_diameter, tooth_type, teeth_count, teeth_count_text, module, module_text, pressure_angle, profile_shift_coefficient, span_teeth_count, common_normal_length, common_normal_length_text, pin_diameter, pin_span, suggested_material_shape, minimum_material_size, expected_scrap_type, expected_scrap_theoretical_size, expected_scrap_usable_size, confidence, need_manual_review。
"""
...
def _fallback_from_candidates(candidates: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "tooth_type": gear.get("tooth_type"),
        "teeth_count": gear.get("teeth_count"),
        "teeth_count_text": gear.get("teeth_count_text"),
        "module": gear.get("module"),
        "module_text": gear.get("module_text"),
        ...
        "common_normal_length_text": gear.get("common_normal_length_text"),
...
def _apply_conservative_rules(candidates: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in ("product_thickness", "plate_thickness", "tooth_type", "teeth_count", "teeth_count_text", "module", "module_text", "pressure_angle", "profile_shift_coefficient", "span_teeth_count", "common_normal_length", "common_normal_length_text", "pin_diameter", "pin_span"):
        if gear.get(field) is not None:
            result[field] = gear[field]
    result["tooth_type"] = normalize_tooth_type(result.get("tooth_type"))
    if result.get("teeth_count_text") and result.get("teeth_count") is None:
        result["teeth_count"] = first_int_value(result["teeth_count_text"])
    if result.get("module_text") and result.get("module") is None:
        result["module"] = plain_float_value(result["module_text"])
